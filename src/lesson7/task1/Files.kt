...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (sub in substrings) {
        ...
        while (ind != -1) {
            map[sub] = map[sub]!! + 1
            ind = read.indexOf(sub.toLowerCase(), ind + 1)
        }
    }
    return map
}
...
fun top20Words(inputName: String): Map<String, Int> {
    ...
    for (words in File(inputName).readLines()) {
        for (word in Regex("""[^a-zA-Zа-яА-ЯёЁ]""").split(words).filter { it != "" }) {
            map[word.toLowerCase()] = map.getOrDefault(word.toLowerCase(), 0) + 1
        }
    }
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    for (word in File(inputName).readLines()) {
        ...
        if (word1.length == word1.toSet().size) max = word1.length
    }
    for (word in File(inputName).readLines()) {
        if (word.length == word.toLowerCase().toSet().size && word.length == max) list.add(word)
    }
